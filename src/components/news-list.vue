         ...
         <li class="goods-item" v-for="item, key in newslist" :key="key" @click="gotoDetail(item)">
           ...
           <div class="detail" :class="{'noImg_detail':item.first_imgsmall==0}">
             <div class="goods-info">
               <div class="title" v-html="item.title" :class="{'noRead':(alias.indexOf('home')==-1&&userInfo.user_id!=item.publish_user&&item.read_flg==0)||(item.read_num==0&&(item.read_end_flg==0||(item.read_end_flg==1&&item.read_end_days>0)))}"></div>
               <div class="meta">
